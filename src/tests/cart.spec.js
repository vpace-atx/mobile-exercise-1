...
describe('Cart workflow tests', () => {
    ...
    it('user can add and remove item from cart', async () => {
    ...
    it('user can add and subtract duplicate item from cart', async () => {
        await ProductPage.addItemToCart();
        await NavigationBar.openCart();
    ...
    it('cart contents and price accurately reflect what user added', async () => {
        ...
        if (driver.isIOS) expect(await CartPage.totalPriceTextIOS(calculateTotalPrice(quantity, 29.99))).toHaveText(`$${calculateTotalPrice(quantity, 29.99)}`);
        else expect(await CartPage.totalPriceTextAndroid).toHaveText(`$${calculateTotalPrice(quantity, 29.99)}`);
    })
})
